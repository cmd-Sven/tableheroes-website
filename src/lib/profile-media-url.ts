...
/** Fraktions-Banner: `{userId}/factions/{worldId}/{segment}/banner-{ts}.webp` */
export function classifyProfileMediaFactionBanner(
  publicUrl: string,
  userId: string,
): NpcPortraitStorageKind | null {
  const path = extractProfileMediaObjectPath(publicUrl);
  ...
  const fileName = parts[parts.length - 1] ?? "";
  if (!fileName.startsWith("banner-")) return null;
