...
export async function getGmCampaignMembersWithCharacters(
  ...
  if (membersWithoutChar.length > 0) {
    ...
    let { data: fallbackChars } = await supabase
      .from("characters")
      .select("id, name, class, race, level, status, biography, avatar_url, token_url, token_storage_path, condition_tokens, mood_state, mood_tokens, active_conditions, user_id, campaign_id, culture_lore_id, languages, faction_membership, current_location_id")
  ...
  const [usersRes, charsRes] = await Promise.all([
    ...
    charIds.length > 0
      ? supabase
          .from("characters")
          .select(
            "id, name, class, race, level, status, biography, avatar_url, token_url, token_storage_path, condition_tokens, mood_state, mood_tokens, active_conditions, culture_lore_id, languages, faction_membership, current_location_id",
          )
          .in("id", charIds)
