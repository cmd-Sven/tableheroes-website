...
export type WorldDashboardData = {
  ...
  lastBestarium: { id: string; name: string } | null;
};
...
export async function getWorldDashboardData(worldId: string): Promise<WorldDashboardData> {
  ...
  if (!user)
    return {
      pendingTasksByType: { npc: 0, location: 0, faction: 0 },
      lastNpc: null,
      lastLocation: null,
      lastLore: null,
      lastFaction: null,
  ...
  const [worldTasks, npcRes, locationRes, loreRes, factionRes, bestariumRes] = await Promise.all([
  ...
  return {
    ...
    lastBestarium: bestariumRes?.data ? { id: bestariumRes.data.id, name: bestariumRes.data.name } : null,
  };
}
