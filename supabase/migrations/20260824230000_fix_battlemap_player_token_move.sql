-- Fix: Spieler-Token-Bewegung — Session-Zugriff und Charakter-Ownership an Kampagnenmitgliedschaft anbinden.
...
COMMENT ON FUNCTION public.user_can_access_session(uuid) IS
  'Session-Zugriff für GM/Owner oder Kampagnenmitglieder (inkl. Accepted).';

-- Ownership: campaign_members-Zuweisung oder characters.user_id
...
BEGIN
  ...
  IF v_is_gm THEN
    v_can_move := true;
  ELSIF v_char.user_id = v_uid THEN
    v_can_move := true;
  ELSE
    SELECT EXISTS (
      SELECT 1
...
COMMENT ON FUNCTION public.place_battlemap_character_token(uuid, uuid, uuid, integer, integer, boolean) IS
  'Spieler/SL: Charakter-Token platzieren oder verschieben. Ownership via characters.user_id oder campaign_members.';
